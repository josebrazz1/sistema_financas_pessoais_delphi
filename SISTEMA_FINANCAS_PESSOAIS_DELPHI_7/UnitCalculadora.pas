...
var
  ...
  ultimoChar : String;
begin
  {Quando está clicando em algum número significa que não está apagando nada,
  ...
  begin
    ...
    {Pega o caracter contido na última posição da string}
    ultimoChar := txtCalc.Text[txtCalcLength];
  end
  {Caso o visor estiver vazio não se pode digitar a vírgula, somente um número}
  else if (Sender as TBitBtn).Caption = ',' then
    Exit;

  {Caso NÃO exista operador, e o texto do valor1 já possua uma vírgula, e o botão
  pressionado for a vírgula e o segundo valor for vazio}
  {Neste caso o usuário está tentando colocar duas vírgulas no primeiro valor}
  if (not operadorClicado) and (AnsiContainsStr(txtValor1.Text, ',')) and ((Sender as TBitBtn).Caption = ',') and (txtValor2.Text = '') then
    Exit;

  {Caso EXISTA operador, e o texto do valor2 já possua uma vírgula, e o botão
  pressionado for a vírgula e o segundo valor NÃO for vazio}
  {Neste caso o usuário está tentando colocar duas vírgulas no segundo valor}
  if (operadorClicado) and (AnsiContainsStr(txtValor2.Text, ',')) and ((Sender as TBitBtn).Caption = ',') and (txtValor2.Text <> '') then
    Exit;

  {Se o botão sendo apertado for a vírgula e o último caracter também for vírgula}
  if ((Sender as TBitBtn).Caption = ',') and (ultimoChar = ',') then
    {Então encerra o código}
    {aqui valida que não haja duas virgulas digitadas, por exemplo: "5,,"}
    Exit;

  {Se o botão sendo apertado for a vírgula e o último caracter for um operador}
  if ((Sender as TBitBtn).Caption = ',') and (ultimoChar = operador) then
    {Então encerra o código}
    {aqui valida que não haja uma vírgula digitada após um operador,
    por exemplo: "+,"}
    Exit;

  {Se nenhum operador foi selecionado para o cálculo ainda}
  if not operadorClicado then
    {Então atribui valor para o primeiro valor}
...
begin
  ...
  {Neste ponto o usuário está clicando em algum operador na calculadora}
  if (not operadorClicado) and (txtValor1.Text <> '') then
  begin
    {Seta que ele escolheu algum operador}
    operadorClicado := True;
    {O sinal do operador e passado por parâmetro}
    operador := prOperador;
    {Pega o valor que já está na tela e adiciona o operado ao lado direito}
    txtCalc.Text := txtCalc.Text + operador;
    {Passa qual o índice do operador para usar no case da procedure "calcular"}
    operadorIndice := operadorIndexCase;
  end
  else
  begin
    {Verifica em qual botão de operador o usuário clicou e compara com o operador
    selecionado atualmente}
    if operador = prOperador then
    begin
      {Caso sejam iguais realiza o cálculo daquele operador novamente}
      
      {Neste ponto o usuário selecionou dois número e em vez de clicar no
      sinal de igual clicou no opetrador novamente, assim funcionará como se
      tivesse clicano no sinal de igual}
      divZero := False;
      {Realiza o cálculo}
      calcular(valor1, valor2, operadorIndexCase);
      {Verifica se a operação não está resultando em uma divisão por zero}
...
begin
  {Parte sempre do pressuporto que o cálculo não é uma divisão por zero, mas
  caso seja será identificado afrente}
  divZero := False;

  {Verifica o índice do operador e realiza as operações abaixo conforme a
  combinaçao deles}
  case indiceOperador of
    ...
    begin
      ...
      divisão, ela não é por zero}
      if valor2 > 0 then
        resultado := RoundTo((valor1 / valor2),-2)
      else
      begin
        {Caso seja divisão por zero, mostra o erro na tela da calculadora e seta
        a booleana para False}
        txtCalc.Text := 'Div/0';
        divZero := True;
      end;
    end;
    else ShowMessage('Erro ao realizar o cálculo com o operador: '+ operador);
  end;
...
begin
  {Se não ocorreu uma divisão por zero}
  if not divZero then
  begin
    {Limpa o valor2 para que possa ser realizado um novo cálculo}
    valor2 := 0;
    {O visor da calculadora mostra o resultado}
    txtCalc.Text := FloatToStr(resultado);
    {Essa variável é setada para false e ficará pronta para outro cálculo}
    operadorClicado := False;
    operador := '';
    {Este campo mantém o resultado para cálculo futuro}
    txtValor1.Text := FloatToStr(resultado);
    txtValor2.Text := '';
    {Normaliza o estado de divisão por zero, para que não haja erro nos próximos
    cálculos}
    divZero := False;
  end
  else
  begin
    {Entra aqui o caso de uma divisão por zero}
    operadorClicado := False;
    operador := '';
    txtValor1.Text := '';
    valor1 := 0;
...
begin
  {Quando se clica no botão de igual é verificado se o campo do valor1 não
  é vazio e se existe  algum operador selecionado. No caso dessa condição ser
  falsa significa que o usuário ainda não selecionou nenhum valor, muito menos
  um operador para o cálculo}
...
begin
  {Se o usuário está tentando realizar uma divisão por zero entra neste if}
  if (operador = '/') and (valor2 = 0) then
    divZero := True;

  {Se o valor2 não for vazio, significa que o cálculo será feito por este botão}
  if txtValor2.Text <> '' then
  begin
    {E o cálculo não for uma divisão por zero}
    if not divZero then
    begin
      {Realiza o cálculo}
      calcular(valor1, valor2, operadorIndice);
      {Manda o resultado do cálculo para os campos valor dos formulários}
    ...
    begin
      {Entra aqui se é uma divisão por zero}
      txtCalc.Text := 'Div/0';
      operadorClicado := False;
      operador := '';
      txtValor1.Text := '';
  ...
  {Caso o valor2 for vazio, significa que  cálculo já está feito}
  else
  begin
    {Verifica qual formulário está ativo e manda para o campo valor}
    formCadReceita.txtValor.Text := FloatToStr(resultado);
    formCadTransacao.txtValor.Text := FloatToStr(resultado);
    Close;
  end;
...
begin
  ...
  txtValor1.Text := '';
  txtValor2.Text := '';
  {diz que não há nenhum operador selecionado}
  operadorClicado := False;
  {Limpa o sinal de operador}
  operador := '';
  {Zera o índice do operador}
  operadorIndice := 0;
end;
