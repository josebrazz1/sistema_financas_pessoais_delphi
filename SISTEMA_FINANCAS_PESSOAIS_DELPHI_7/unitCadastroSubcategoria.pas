...
begin
  ...
  if formAberto = nmFORM_RECEITA then
  begin
    alimentaCombo('SELECT NOME_SUBCATEGORIA FROM SUBCATEGORIA WHERE TIPO_CATEGORIA = "RECEITA" OR TIPO_CATEGORIA = "AMBOS" ORDER BY CÓDIGO', formCadReceita.cbSubcategoria);
    formCadReceita.cbSubcategoria.ItemIndex := formCadReceita.cbSubcategoria.Items.Count - 1;
  end
  else if formAberto = nmFORM_DESPESA then
  begin
    alimentaCombo('SELECT NOME_SUBCATEGORIA FROM SUBCATEGORIA WHERE TIPO_CATEGORIA = "DESPESA" OR TIPO_CATEGORIA = "AMBOS" ORDER BY CÓDIGO', formCadTransacao.cbSubcategoria);
...
begin
  ...
  begin
    ShowMessage('A descrição da subcategoria não pode ser vazia');
    ModalResult := mrNone;
  end;
end;
