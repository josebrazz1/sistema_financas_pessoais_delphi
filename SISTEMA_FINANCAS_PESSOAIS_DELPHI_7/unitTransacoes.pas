...
//Insere transacoes com cartão de crédito no banco de dados ===========================================
...
begin
  ...
  begin
    ...
    begin
      ...
      {Se o mês da parcela que está sendo inserida no momento for menor que 12,
      ou seja, menor que dezembro, o mês é incrementado.}
      if mesData < 12 then
      begin
        mesData := mesData + 1;
      end
      {Caso contrário, se o mês for igual a 12 (dezembro) então a variável recebe 1
      novamente. Isso acontece para não deixar passar valores maiores que 12, o que
      causaria erro, pois não há nenhum mês após dezembro}
      else
      begin
        mesData := 1;
        anoData := anoData + 1;
...
//Deleta transacoes de cartão de crédito do banco de dados ===========================================
...
begin
  ...
  begin
    ...
    begin
      for i := 0 to slExclusao - 1 do
      begin
        dmDados.dataSet.CommandText := 'SELECT ID_CCR FROM DADOS_INTEGRADOS WHERE IDENTIFICAÇÃO = ' + cbTransacoesExclusao.Items[i];
        dmDados.dataSet.Open;
        idCcr := dmDados.dataSet.FieldByName('ID_CCR').AsInteger;
        dmDados.dataSet.Close;
    ...
    begin
      dmDados.dataSet.CommandText := 'SELECT ID_CCR AS ID_CCR FROM DADOS_INTEGRADOS WHERE IDENTIFICAÇÃO = ' + cbTransacoesExclusao.Items[0];
      dmDados.dataSet.Open;
      idCcr := dmDados.dataSet.FieldByName('ID_CCR').AsInteger;
      dmDados.dataSet.Close;
...
begin
  ...
  alimentaCombo('SELECT NOME_CATEGORIA FROM CATEGORIA WHERE TIPO_CATEGORIA = "DESPESA" OR TIPO_CATEGORIA = "AMBOS" ORDER BY CÓDIGO', cbCategoria);
  alimentaCombo('SELECT NOME_SUBCATEGORIA FROM SUBCATEGORIA WHERE TIPO_CATEGORIA = "DESPESA" OR TIPO_CATEGORIA = "AMBOS" ORDER BY CÓDIGO', cbSubcategoria);
...
begin
  ...
  {Remove a formatação de moeda da coluna 4 da grid, adim de não dar nenhum
  erro de conversão ao realizar os procedimentos abaixo}
  removeConcStringNaStringGrid(stringGridTransacoes, 4);

  {Caso a linha clicada na grid não seja o cabeçalho}
  if Row > 0 then
  begin
    {Caso a linha clicada não esteja selecionada, adiciona o X no símbolo de
    check "[ X ]"}
    if stringGridTransacoes.Cells[0,Row] <> '[  X  ]' then
    begin
      stringGridTransacoes.Cells[0,Row] := '[  X  ]';

      {Adiciona o id do ítem da linha selecionada na combo box de transações
      para deletar}
    ...
    {Caso a linha clicada já esteja selecionada, retira o X no símbolo
    de check "[ X ]"}
    else
    begin
      stringGridTransacoes.Cells[0,Row] := '[    ]';

      {Remove o id do ítem da linha selecionada da combo box de transações
      para deletar}
  ...
  {Inicializa a variável contadora de ítens para exclusão}
  slExclusao := 0;
  for i := 0 to stringGridTransacoes.RowCount - 1 do
  begin
    {Para cada linha clicada na grid soma 1 na variável contadora de ítens para
    xclusão}
    if stringGridTransacoes.Cells[0, i] = '[  X  ]' then
      slExclusao := slExclusao + 1;
  end;

  {Caso exista ítens selecionados na grid}
  if slExclusao > 0 then
  begin
    {Altera o nome do botão deletar de acordo com o número de ítens}
    {Se estiver marcado 1 ítem o nome será: Deletar 1 transação}
    {Se estiver marcado 2 ítens o nome será: Deletar 2 transações}
    {E assim por diante}
    if slExclusao = 1 then
  ...
  begin
    {Caso não exista nenhum ítem selecionado na grid}
    {Mantém apenas "Deletar" como nome do botão deletar}
  ...
  {Manda o codigo da linha selecionada "ID" para a variável id}
  if slExclusao > 0 then
    id := StrToInt(stringGridTransacoes.Cells[1,row])
  else
    cancelarAlteracao();

  {Caso haja mais de 1 ítem selecionado na grid desabilita a opção de alteraçao,
  pois permite-se apenas a alteração de uma única transação por vez}
  ...
  begin
    {Caso o ítem na posição "i" da combo box de ítens para exclusão for
    diferente de vazio}
    if cbTransacoesExclusao.Items[i] <> '' then
    begin
      {Seleciona o tipo da transação do item atual}
      {O tipo de transação diferencia as transações Normais, Fixas e de Cartão
      de crédito}
  ...
  {Se a combobox de transações para exclusão não estiver vazia}
  ...
  begin
    ...
    begin
      ...
      {Seleciona a transação que está na primeira posição da combobox (posição zero)}
      ...
      begin
        {Preenche os campos do formulário com os dados retornados na consulta}
      ...
      begin
        rdCcr.Checked := True;
        {Caso a transação for de cartão de crédito recupera também o número de
        parcelas}
...
begin
  ...
  try
    ...
    if ckCcr.Checked then
      ShowMessage('Ainda não foi criado a função de alterar despesas de cartão de crédito');
      //insereDespesaBdCcr(txtData.Text, cbConta.Text, txtDescricao.Text, StrToFloat(txtValor.Text), cbCategoria.Text, cbSubcategoria.Text, cbPagoRecebido.Text, StrToInt(txtParcelas.Text));

    if ckFixa.Checked then
      ShowMessage('Ainda não foi criado a função de alterar despesas fixas');
      //insereDespesaBdFixa(txtData.Text, cbConta.Text, txtDescricao.Text, StrToFloat(txtValor.Text), cbCategoria.Text, cbSubcategoria.Text, cbPagoRecebido.Text);
